...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', async () => {
    try {
      ...
      if (reg.waiting) { console.info('[PWA] Update ready.'); showUpdateToast(); }
      reg.addEventListener('updatefound', () => {
        const nw = reg.installing;
        nw && nw.addEventListener('statechange', () => {
          if (nw.state === 'installed' && navigator.serviceWorker.controller) { console.info('[PWA] Update installed.'); showUpdateToast(); }
        });
      });
